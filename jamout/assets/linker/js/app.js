...
function cometMessageReceivedFromServer(message) {
  ...
  // This message has to do with Room Model
  if (message.model === 'room') {
        var roomId = message.id
        updateRoomInDom(roomId, message);
      }
...
function updateRoomInDom(roomId, message) {
  ...
  // Route to the appropriate room update handler based on which page you're on
  switch (true) {

     //User Room index page
      case /\/room/.test(page):
               
        //This is a message coming from publishCreate
        if (message.verb === 'create'){
          //console.log("Messsage from room create received!");
          RoomIndexPage.addRoom(message);
          }
        //This is a message coming from publishDestroy
        if (message.verb === 'destroy') {
        RoomIndexPage.destroyRoom(roomId);
       }

      break;

    //User Profile Page
    //case /\/user\/(show\/)?(\w+)?(\d+)?/.test(page):
...
////////////////////////////////////////////
  // Room index page DOM manipulation logic
  // (i.e. backbone-style view)
////////////////////////////////////////////
...
var RoomIndexPage = {
     ...
     //Add a room to the list of rooms in the Room Administration Page
     addRoom: function(room) {

    // obj is going to encompass both the new room data as well as the _csrf info from
    // the layout.ejs file
    var obj = {
      room: room.data,
      _csrf: window.jamout.csrf || ''
    ...
    // Add the template to the bottom of the Room Administration Page
    $( 'tr:last').after(

        // This is the path to the templates file
        JST['assets/linker/templates/addRoom.ejs']( obj )
      );
  },

  // Remove the room from the Room Administration Page
